...
| **IndexField** | Optional | `string` | Name of the time field. JSONPath, XPath, and CSV are supported<sup>1</sup>. For CSV files without a header, the column index (`1` being the first column) should be specified.<br><br>Allowed values if no timestamp is provided in the file:<br>  - **FileCreationTime** - UTC time when the file was created. <br> - **FileModifiedTime** - UTC time when the file was modified.<br> - Any valid JSONPath, XPath, or CSV column number or header<br><br>Example: \"FanSpeedTimeStamp\"|
