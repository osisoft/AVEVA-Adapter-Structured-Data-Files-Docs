...
| **IndexField** | Optional | `string` | Name of the time field. JSONPath, XPath, and CSV are supported<sup>1</sup>. For CSV files without a header, the column index (`1` being the first column) should be specified.<br><br>Default value: **AdapterGeneratedTime** - UTC time the file is processed by the adapter.<br><br>Allowed values if no timestamp is provided in the file:<br>  - **FileCreationTime** - UTC time when the file was created. <br> - **FileModifiedTime** - UTC time when the file was modified.<br> - Any valid JSONPath/XPath and column number or header for CSV<br><br>Example: \"FanSpeedTimeStamp\"|
