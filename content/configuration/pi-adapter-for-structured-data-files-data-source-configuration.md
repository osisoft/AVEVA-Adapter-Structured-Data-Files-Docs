...
| api/v1/configuration/\<ComponentId\>/DataSource | `POST` | Creates the data source configuration. After the request is received, the adapter starts collecting data. `POST` cannot be used again until the current data source configuration is deleted. |
| api/v1/configuration/\<ComponentId\>/DataSource | `PUT` | Configures or updates the data source configuration. Overwrites any data source configuration currently in effect. If no configuration is in effect, the adapter starts collecting data after it receives the request. |
